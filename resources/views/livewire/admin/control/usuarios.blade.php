<div>
    <section class="flex flex-col md:flex-row gap-5 items-end mb-5">
        ...
        <div class="flex flex-col w-full md:w-auto">
            <label for="perProfecion">Profecion:</label>
            <x-select wire:model.change="perProfecion">
                <option value="">{{ __('All') }}</option>
                @forelse ($tipoUsuarios as $item)
        ...
        <div class="flex flex-col w-full md:w-auto">
            <label for="perCaducidad">Fecha:</label>
            <x-input type="date" wire:model.change="perCaducidad" />
        </div>
    ...
    <x-table>
        ...
        <x-slot name="rows">
            <x-loading-spinner :colspan="6" wire:target="search,perPage,perEstatus,perCaducidad,perProfecion" />
            @forelse ($collection as $item)
                <x-tr>
                    ...
                    <x-td-row>
                        {{ __($item->tipoUsuario->nombre) }}
                    </x-td-row>
                    <x-td-row>
                        {{ $item->profecional->fecha_pago ?? 'Sin subscripción' }}
                    </x-td-row>
                    <x-td-row>
            ...
            @empty
                <x-tr>
                    <x-td-row colspan="6" class="text-center">
                        Sin resultados
                    </x-td-row>
                </x-tr>
            @endforelse
